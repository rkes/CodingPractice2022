...
public class RestoreIP_practice {

    public static void main(String[] args) {

    }
    public static void buildIps(String ip , List<String> ips,int segment,int idx){
        if(segment >= ip.length()){
            return;
        }
        for(int i=1;i<=3 && i<=ip.length()+segment;i++){
            String str = ip.substring(i,i+segment);
            if(isValidIP(str)){
                ips.add(str);
                buildIps(ip.substring(i+segment),ips,segment+1,idx+1);
            }
        }
    }
